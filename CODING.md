...
Четыре права на категорию (`public`/`private`/`session`), задаются в `users.php`:

| Право    | Метод             | Что разрешает                              |
|----------|-------------------|--------------------------------------------|
| `use`    | `canUse()`        | Видеть репозитории в списке                |
| `edit`   | `canEdit()`       | Добавлять, редактировать, перемещать       |
| `init`   | `canInit()`       | Инициализировать новые restic-репозитории   |
| `delete` | `canDelete()`     | Удалять репозитории                        |

- `edit` подразумевает `use`
- `canMove(from, to)` требует `edit` на обе категории
- Чекбокс «init» в форме добавления показывается только если `canInit` для выбранной категории
