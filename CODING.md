...
- **Комментарии добавляются ОТДЕЛЬНОЙ строкой** — не дописывайте их в конец
  существующей строки кода. Так git опознает комментарий как добавленную строку,
  а не как правку существующей, и diff остаётся минимальным и читаемым.
- Комментарии пишутся по-русски, объясняют «зачем», а не «что» (код сам говорит «что»).
- Обязательны комментарии для нетривиальных решений, workaround'ов и осознанных
  отступлений от очевидного поведения (см. Gotchas в AGENTS.md).
...
- `password_var` хранит **имя** переменной/секрета; значение переменной — это
  **bcrypt-хеш** пароля (открытые пароли не храним).
...
- Для `admin` дефолтное имя — `PHPRESTICADMIN_ADMIN_PASSWORD_HASH`; остальные
  пользователи задают произвольное имя сами.
